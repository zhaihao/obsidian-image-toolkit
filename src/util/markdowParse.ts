...
export const parseActiveViewData = (plugin: ImageToolkitPlugin, lines: string[], file: TFile): GalleryImgCacheCto => {
  ...
  for (let i = 0, len = imgList.length; i < len; i++) {
    ...
    if (img.convert) {
      ...
      // 修复 http 图片无法显示缩略图的问题
      img.src = imageFile ? plugin.app.vault.getResourcePath(imageFile) : (img.src.toLowerCase().startsWith("http") ? img.src:'');
    }
...
const IMAGE_LINK_REGEX2 = /\[\s*?(!\[\[(.*?[jpe?g|png|gif|bmp|tiff|svg|webp|heif|avif].*?)]])\s*?]\(.*?\)/i; // 2-link: [ ![[src|alt1|alt2|width]] ](https://...)
const IMAGE_REGEX2 = /!\[\[(.*?[jpe?g|png|gif|bmp|tiff|svg|webp|heif|avif].*?)\]\]/i; // 2: ![[src|alt1|alt2|width]]

const SRC_LINK_REGEX = /[a-z][a-z0-9+\-.]+:\/.*/i; // match link: http://, file://, app://
const SRC_IMG_REGREX = /.*?\.jpe?g|png|gif|bmp|tiff|svg|webp|heif|avif/i; // match image ext: .jpg/.jpeg/.png/.gif/.svg/.bmp
...
const FULL_PATH_REGEX = /^[a-z]\:.*?[jpe?g|png|gif|bmp|tiff|svg|webp|heif|avif]|/i;
...
const matchImage1 = (text: string): GalleryImgCto => {
  ...
  // 过滤掉 src 不是图片格式的
  let typeMatch = img.src.toLowerCase().match(".*\\.(jpe?g|png|gif|bmp|tiff|svg|webp|heif|avif)")
  if(!typeMatch) {
    return null;
  }
  // 没有 alt 的图片会导致 click image index 为 0
  if (img.alt==='') img.alt=img.name
  img.convert = true;
  return img;
}
...
const matchImage2 = (text: string): GalleryImgCto => {
  ...
  // 过滤掉 src 不是图标格式的
  let typeMatch = img.src.toLowerCase().match(".*\\.(jpe?g|png|gif|bmp|tiff|svg|webp|heif)")
  if(!typeMatch) {
    return null;
  }
  // 没有 alt 的图片会导致 click image index 为 0
